...
func (d *DataPack) UnPack(binaryData []byte) (ziface.IMessage, error) {
	...
	//读dataLen
	if err := binary.Read(dataBuff, binary.LittleEndian, &message.MsgLen); err != nil {
		return nil, err
	}

	//读msgID
	if err := binary.Read(dataBuff, binary.LittleEndian, &message.MsgId); err != nil {
		return nil, err
	}

	//判断dataLen的长度是否超出我们允许的最大包长度
	if utils.GlobalObject.MaxPacketSize > 0 && utils.GlobalObject.MaxPacketSize < message.MsgLen {
